...
# ================================================================
# NOTE: This bot uses Telegram long polling (getUpdates).
#       It does NOT open any port, so it runs safely alongside
#       your other 4-5 bots without any port conflicts.
# ================================================================
...
# ── Function: Find a free port (if ever needed in future) ───────
find_free_port() {
    local port
...
echo "[INFO] Checking port availability on this VPS..."
echo "[INFO] Currently used ports:"
ss -tlnp | grep LISTEN | awk '{print $4}' | sort -u
echo ""
FREE_PORT=$(find_free_port)
echo "[INFO] Next free port available: $FREE_PORT"
echo "[INFO] (This bot uses polling, so no port is needed. Stored for reference.)"
echo ""
...
echo "[2/7] Installing Python 3.11+ and system dependencies..."
apt install -y \
    python3 \
    python3-pip \
    python3-venv \
...
echo "[3/7] Setting up project directory at $PROJECT_DIR..."
...
echo "================================================"
echo ""
echo " This bot uses POLLING — no port needed."
echo " Safe to run alongside your other bots."
echo " (Free port $FREE_PORT saved for future reference)"
echo ""
echo " Quick Commands:"
echo "   Start:   sudo systemctl start pocketfm-bot"
echo "   Stop:    sudo systemctl stop pocketfm-bot"
echo "   Restart: sudo systemctl restart pocketfm-bot"
echo "   Status:  sudo systemctl status pocketfm-bot"
echo "   Logs:    sudo journalctl -u pocketfm-bot -f"
echo "   App Log: tail -f $PROJECT_DIR/bot.log"
echo ""
